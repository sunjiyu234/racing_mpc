...
#include <algorithm>
#include <random>
#include <learningmpc_nonlinear/spline.h>
#include <learningmpc_nonlinear/CSVReader.h>
#include <Eigen/Dense>
#include <nav_msgs/OccupancyGrid.h>
#include <learningmpc_nonlinear/occupancy_grid.h>
...
class Track{
public:
    vector<Point_ref> centerline_points;   // X Y  THETA phi 
    tk::spline X_spline;
    tk::spline Y_spline;
    double length;
    double space;
    ...
    Track(string file_name, bool sparse=false) : space(0.05){
        ...
        if (sparse) {
            ...
            while (s<length){
                ...
                p_new.phi = getPhi(s);
                p_new.theta = s;
                // p_new.kr = getcenterline_pointsCurvature(s);  // don't need kr;
