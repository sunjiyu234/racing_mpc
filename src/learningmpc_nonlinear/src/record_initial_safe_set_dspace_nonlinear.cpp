...
Record_SS::Record_SS(ros::NodeHandle &nh) : nh_(nh) {
    ...
    data_file_.open( "/home/sun234/racing_work/src/ltv_mpc/src/initial_safe_set_dspace_new_10ms_ed_5.csv");
}
...
void Record_SS::cmd_callback(const ackermann_msgs::AckermannDriveStampedConstPtr &cmd_msg) {
    ...
    if (cmd_msg != nullptr){
        ...
        double Tf_cmd_init = cmd_msg->drive.speed;
        double Tr_cmd_init = cmd_msg->drive.acceleration;
        double steer_cmd_init = cmd_msg->drive.steering_angle  * M_PI / 180.0;
        ...
        double Tf_cmd = (Tf_cmd_init - Tf) / 0.05;
        double Tr_cmd = (Tr_cmd_init - Tr) / 0.05;
        double steer_cmd = (steer_cmd_init - delta) / 0.05;
...
int main(int argc, char **argv){
    ros::init(argc, argv, "record_init_ss_nonlinear");
    ros::NodeHandle nh;
    Record_SS rec_ss(nh);
    ros::spin();
    return 0;
