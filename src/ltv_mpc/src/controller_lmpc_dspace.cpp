...
LMPC::IterativeController gController;  //内含两个全局函数
double kp = 30.0;
double ki = 30.0;
double kd = 0.0;
...
//初始化LMPC
void initializeLMPC(){
  LMPC::Parameters parameters;
  parameters.dt = 0.05;
  parameters.pred_horizon = 20;
  parameters.control_horizon = 10;

  LMPC::Model model;
  model.l_f = 1.015;   //质心前轴距
  model.l_r = 1.895;   //质心后轴距
  model.m = 1270.0;       //车辆质量
  model.I_z = 1536.7;        //车辆横摆惯量*
  model.l = 2.91;      //车辆轴距
  model.h = 0.54;         //车辆质心高*
  model.g = 9.8;
  model.vx = v_target;
  ...
  LMPC::HardConstraint constraint;
  ...
  LMPC::CostFunctionWeights weights;
  weights.w_r = 0.0;
  weights.w_v_y = 0.5;
  weights.w_e_yaw =  1.0;
  //weights.w_e_d = 0.75;
  weights.w_e_d = 2.0;
  weights.w_steer_angle = 500.0;
...
{
  ros::init(argc, argv, "controller_lmpc");
  ros::NodeHandle nh;
  cout << "here" <<endl;
  //发送底盘控制数据topic
  ...
  std::string track_filename = nh_priv.param<std::string>("track_filename", "/home/nvidia/vehicle_work/src/vehicle_work/src/double_line_change.txt");
...
/*
    ...
    */
    stringstream ss_pre_vy, ss_pre_wr;
    ss_pre_vy << gState.vy;
    ss_pre_wr << gState.r;
...
*/
    //calculate Lmpc
    LMPC::ControlOutput control_output;   //存放steer
    std::vector<LMPC::State> prediction_output;
    ...
    control_a_constraint = max(min(5.0, control_a), -5.0);
    double control_T;
    ...
    string s_state, s_x, s_y, s_v, s_yaw, s_r, s_control_v, s_control_steer,s_error,s_vy,s_yaw_rate,s_num_error,s_v_east,s_v_north,s_time,s_time_get_path,s_time_update,s_e_yaw,s_T,s_pre_vy,s_pre_wr;
    string comma = ",";
    ss_x << gState.x;
    ss_y << gState.y;
    ss_v << gState.vx;
    ...
    ss_r << gState.r;
    ss_control_v << cmd_vel.twist.linear.x;
    ss_control_steer << cmd_vel.twist.angular.x;
    ...
    //s_state = s_error;
    s_state = comma+s_pre_vy+comma+s_pre_wr+comma+s_r + comma +s_vy + comma+ s_v + comma+ s_yaw+comma+ s_x+comma+s_y+comma+s_control_steer+comma + s_T +comma+s_num_error+comma+s_error + comma + s_e_yaw + comma + s_time + comma + s_time_get_path + comma + s_time_update;
    ...
    if (abs(sqrt(pow(gState.x - gPlanner.goal_x_, 2) + pow(gState.y - gPlanner.goal_y_, 2))) < 0.25)
    {
      ...
      cmd_vel.twist.linear.x = -100001; //到终点后发送0车速，0转角
