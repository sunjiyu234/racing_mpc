...
#include <Eigen/Core>
#include <tuple>
#include <controller_mpc/track.h>
...
string waypoint_file = "/home/sun234/racing_work/src/learningmpc_nonlinear/data/racing_2_source.csv";
Track* track_ =  new Track(waypoint_file, true);
double s_prev_ = 0.0;
double s_curr_ = 0.0;
double vehicle_v;
double vehicle_beta;
int iter_;
int time_;
vector<double> carsim_s_list, carsim_v_list;
double carsim_x, carsim_y, carsim_v, carsim_s, carsim_s_prev;
bool first_receive_ = true;
int lap_ = 0;
double control_T_last;
...
void add_point(double control_T, double control_dert){
  Sample point;
  point.x << gState.x, gState.y, gState.yaw, vehicle_v, gState.r, vehicle_beta;
  point.s = s_curr_;
  point.iter = iter_;
  point.time = time_;
  point.u = {control_T / 2.0, control_T / 2.0, control_dert};
...
void state_callback(const nav_msgs::Odometry::ConstPtr& msgs)
{
  ...
  carsim_x = CAN_data.pose.pose.position.x;
  carsim_y = CAN_data.pose.pose.position.y;
  carsim_v = sqrt(CAN_data.twist.twist.linear.x * CAN_data.twist.twist.linear.x + CAN_data.twist.twist.linear.y * CAN_data.twist.twist.linear.y);
  if (first_receive_){
    carsim_s = track_->findTheta(carsim_x, carsim_y, 0, true);
    first_receive_ = false;
  }else{
    carsim_s = track_->findTheta(carsim_x, carsim_y, 0, carsim_s_prev, true);
  }
  if (carsim_s - carsim_s_prev < -track_->length/2){
    lap_++;
    for (int index = 0; index < carsim_s_list.size(); ++index){
          cout << carsim_s_list[index] << endl;
    }
    gPlanner.set_cvx(carsim_s_list, carsim_v_list);
    gState.x = 0.0;
    gState.y = 0.0;
    gState.yaw = 0.0;
    gState.vx = gPlanner.spline_s_v_(0.0);
    gState.vy = 0.0;
    gState.r = 0.0;
    vehicle_beta = 0.0;
    vehicle_v = gPlanner.spline_s_v_(0.0);
    marker_spin = 1;
    add_point(control_T_last, 0.0);
    s_prev_ = 0.0;
    s_curr_ = 0.0;
    carsim_s_list.clear();
    carsim_v_list.clear();
  }
  carsim_s_prev = carsim_s;
  carsim_s_list.push_back(carsim_s);
  carsim_v_list.push_back(carsim_v);
...
void vehicle_model(double control_T, double control_dert){
  vehicle_v = sqrt(gState.vx * gState.vx + gState.vy * gState.vy);
  vehicle_beta = asin(gState.vy / vehicle_v);
  gState.x = gState.x + (vehicle_v * cos(gState.yaw + vehicle_beta))*0.05;
  gState.y = gState.y + (vehicle_v * sin(gState.yaw + vehicle_beta)) * 0.05;
  gState.yaw = gState.yaw + gState.r * 0.05;
  nav_msgs::Odometry state_data;
  state_data.pose.pose.orientation = tf::createQuaternionMsgFromYaw(gState.yaw);
  gState.yaw = tf::getYaw(state_data.pose.pose.orientation);
  double Fzf = gController.model_.m * (9.81 * gController.model_.l_r - ax_ * gController.model_.h) / gController.model_.l;
  double Fzr = gController.model_.m * (9.81 * gController.model_.l_f + ax_ * gController.model_.h) / gController.model_.l;
  double Fxf = control_T / (2.0 * gController.model_.r_wheel);
  double Fxr = control_T / (2.0 * gController.model_.r_wheel);
  double alpha_f = -(atan((vehicle_v * sin(vehicle_beta) + gController.model_.l_f * gState.r) / (vehicle_v * cos(vehicle_beta))) - control_dert);
  double alpha_r = -(atan((vehicle_v * sin(vehicle_beta) - gController.model_.l_r * gState.r) / (vehicle_v * cos(vehicle_beta))));
  double Fyf =gController.model_.miu * Fzf * sin(gController.model_.C_f * atan(gController.model_.B_f * alpha_f - gController.model_.E_f * (gController.model_.B_f * alpha_f - atan(gController.model_.B_f * alpha_f))));
  double Fyr = gController.model_.miu * Fzr * sin(gController.model_.C_r * atan(gController.model_.B_r * alpha_r - gController.model_.E_r * (gController.model_.B_r * alpha_r - atan(gController.model_.B_r * alpha_r))));
  double dot_v = (Fxf * cos(control_dert - vehicle_beta) - Fyf * sin(control_dert - vehicle_beta) + (Fxr - gController.model_.Ff - gController.model_.kd * vehicle_v * vehicle_v) * cos(vehicle_beta) + Fyr * sin(vehicle_beta)) / gController.model_.m;
  double dot_r = (gController.model_.l_f * (Fxf * sin(control_dert) + Fyf * cos(control_dert)) - gController.model_.l_r * Fyr) / gController.model_.I_z;
  double dot_beta = -gState.r + (Fxf * sin(control_dert - vehicle_beta) + Fyf * cos(control_dert -  vehicle_beta) - (Fxr - gController.model_.Ff - gController.model_.kd * vehicle_v * vehicle_v) * sin(vehicle_beta) + Fyr * cos(vehicle_beta)) / (gController.model_.m * vehicle_v);
  vehicle_v = vehicle_v + dot_v * 0.05;
  gState.r = gState.r + dot_r* 0.05;
  vehicle_beta = vehicle_beta + dot_beta * 0.05;
  gState.vx = vehicle_v * cos(vehicle_beta);
  gState.vy = vehicle_v * sin(vehicle_beta);
  ax_ = (Fxf * cos(control_dert) - Fyf * sin(control_dert) + Fxr) / gController.model_.m;
}
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher sample_lst_pub = nh.advertise<ltv_mpc::sample_lst>("/sample_lst", 1);
  //cout << "here2"<<endl;
...
//          cout << path_ref <<endl;
  ...
  s_prev_ = track_->findTheta(gState.x, gState.y, 0, true);
  s_curr_ = s_prev_;
...
*/
    ...
    vehicle_model(control_T, control_output.steer);

    s_curr_ = track_->findTheta(gState.x, gState.y, 0, s_prev_, true);
    if (s_curr_ - s_prev_ < -track_->length/2){
      marker_spin = 0;
      update_cost_to_go(curr_trajectory_);
      ltv_mpc::sample send_sample;
      ltv_mpc::sample send_sample_array[curr_trajectory_.size()];
      ltv_mpc::sample_lst send_sample_list;
      cout << send_sample_list.sample_list.size()<<endl;

      for(int index_sample = 0; index_sample < curr_trajectory_.size(); ++index_sample){
        send_sample.x = curr_trajectory_[index_sample].x[0];
        send_sample.y = curr_trajectory_[index_sample].x[1];
        send_sample.yaw = curr_trajectory_[index_sample].x[2];
        send_sample.v = curr_trajectory_[index_sample].x[3];
        send_sample.r = curr_trajectory_[index_sample].x[4];
        send_sample.beta = curr_trajectory_[index_sample].x[5];
        send_sample.tf = curr_trajectory_[index_sample].u[0];
        send_sample.tr = curr_trajectory_[index_sample].u[1];
        send_sample.steer = curr_trajectory_[index_sample].u[2];
        send_sample.s = curr_trajectory_[index_sample].s;
        send_sample.time = curr_trajectory_[index_sample].time;
        send_sample.iter = curr_trajectory_[index_sample].iter;
        send_sample.cost = curr_trajectory_[index_sample].cost;
        send_sample_list.sample_list.push_back(send_sample);
      }
      sample_lst_pub.publish(send_sample_list);
      curr_trajectory_.clear();
    }else{
      add_point(control_T, control_output.steer);
      s_prev_ = s_curr_;
    }

     //sleep and spin
     double clock_final_time = clock();
     cout << "time_calculate = "<<  (clock_final_time-clock_start_time)/CLOCKS_PER_SEC << endl;
     rate.sleep();
  }
}
